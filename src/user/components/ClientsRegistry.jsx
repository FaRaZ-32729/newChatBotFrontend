...
export default function ClientsRegistry({
    ...
    const handleRegisterSubmit = (e) => {
        ...
        try {
            ...
            if (newUser) {
                setCreateUserSuccess(`Successfully registered "${newUser.name}"! Simulated email sent with OTP: ${newUser.otp}`);
                setCreateUserName('');
                setCreateUserEmail('');
            } else {
                                    ...
                                    {managedUsers.map((u) => (
                                        <tr key={u.id} className="hover:bg-slate-900/20 transition-colors">
                                            ...
                                            <td className="p-4 text-center">
                                                <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold border ${u.status === 'active'
