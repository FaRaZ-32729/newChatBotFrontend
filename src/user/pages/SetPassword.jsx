...
export default function SetPassword() {
    ...
    // Load the verified email from verification step
    useEffect(() => {
        const verifiedEmail = localStorage.getItem('otp_verified_email');
        if (!verifiedEmail) {
            // Security guard: Must have completed OTP step
    ...
    const handleSubmit = (e) => {
        ...
        // Simulated premium security lock-in delay
        setTimeout(() => {
            try {
                ...
                if (userIndex === -1) {
                    setError('User profile could not be found. Please restart the process.');
                    setIsLoading(false);
                    return;
                }

                // Lock in new password
                users[userIndex].password = password;
                users[userIndex].otpVerified = true;
                // Clear OTP so it is single-use
                users[userIndex].otp = undefined;

                // Save back
                localStorage.setItem('chatbot_users', JSON.stringify(users));

                // Clear session verification token
                localStorage.removeItem('otp_verified_email');
                localStorage.removeItem('sandbox_last_email_otp'); // Clean sandbox mock inbox

                setSuccess('Password updated successfully! Redirecting you to login portal...');
